...
public:
	...
	bool
	apply_patch_map(
		domain::gridmap::bittable patch, point padded_loc)
	{
		if((uint64_t)padded_loc.x + patch.width() >= (uint64_t)map().width()
		|| (uint64_t)padded_loc.y + patch.height() >= (uint64_t)map().height())
			return false;

		// apply patch
		patch.copy(
			map(), point_to_id(padded_loc),
		...
		domain::gridmap::bittable patch, point padded_loc)
	{
		...
		rgrid_id dest_id = rpoint_to_rid(point_to_rpoint(point(padded_loc.x, padded_loc.y + patch.height() - 1)));
		grid_id src_id = static_cast<grid_id>(patch.xy_to_id(0, patch.height()-1));
